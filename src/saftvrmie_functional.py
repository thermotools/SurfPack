...
class saftvrqmie_functional(saft_dispersion):
    ...
    def calc_non_additive_diameters(self):
        ...
        for i in range(self.nc):
            ...
            for j in range(i+1, self.nc):
                self.delta_ij[i, j], self.delta_T_ij[i, j] = self.thermo.hard_sphere_diameter_ij(
                    i+1, j+1, self.T)
    ...
    def calc_bmcsl_mu(self):
        ...
        for i in range(self.nc):
            ...
            for j in range(i+1, self.nc):
                self.mu_ij[i, j] = self.d_hs[i] * \
                    self.d_hs[j]/(self.d_hs[i]+self.d_hs[j])
                ...
                self.mu_ij_T[i, j] = (self.d_T_hs[i]*self.d_hs[j] + self.d_hs[i]*self.d_T_hs[j])/(self.d_hs[i]+self.d_hs[j]) \
                    - (self.d_T_hs[i] + self.d_T_hs[j]) * \
                    self.mu_ij[i, j]/(self.d_hs[i]+self.d_hs[j])
            ...
            for i in range(self.n_grid):
                ...
                for j in range(self.nc):
                    ...
                    for k in range(j+1, self.nc):
                        ...
                        g_jk, = self.thermo.calc_bmcsl_gij_fmt(
                            n_alpha, self.mu_ij[j, k])
                        f[i] -= 4*np.pi*n0j*n0k*self.d_ij[j, k]**2 * \
                            g_jk*(self.d_ij[j, k] - self.delta_ij[j, k])
        return f
            ...
            for i in range(self.n_grid):
                ...
                for j in range(self.nc):
                    ...
                    for k in range(j+1, self.nc):
                        ...
                        g_jk, g_jk_n, = self.thermo.calc_bmcsl_gij_fmt(
                            n_alpha, self.mu_ij[j, k], calc_g_ij_n=True)
                        ck = -4*np.pi*self.d_ij[j, k]**2 * \
                            (self.d_ij[j, k] - self.delta_ij[j, k])
        ...
        if self.na_enabled:
            for i in range(self.n_grid):
                ...
                for j in range(self.nc):
                    ...
                    for k in range(j+1, self.nc):
                        n0k, _ = get_n0jxi(dens, i, k)
                        g_jk, g_jk_T, = self.thermo.calc_bmcsl_gij_fmt(
                            n_alpha, self.mu_ij[j, k], mu_ij_T=self.mu_ij_T[j, k])
                        ck = -4*np.pi*n0j*n0k
    ...
    def test_eos_differentials(self, V, n):
        ...
        if self.na_enabled:
            ...
            bd = bulk_weighted_densities(
                rho_b=(n/V)*NA*self.grid_reducing_lenght**3, R=self.R, ms=self.ms)
            n_alpha = np.zeros(6)
            n_alpha[:4] = bd.n[:]
                ...
                for k in range(j, self.nc):
                    ...
                    for i in range(6):
                        if abs(n_alpha[i]) > 0.0:
                            ...
                            g_jk_p, = self.thermo.calc_bmcsl_gij_fmt(
                                n_alpha_p, self.mu_ij[j, k])
                            g_jk_m, = self.thermo.calc_bmcsl_gij_fmt(
                                n_alpha_m, self.mu_ij[j, k])
                            print("j,k,alpha", j, k, i, (g_jk_p-g_jk_m) /
                                  (2*n_alpha[i]*eps), g_jk_n[i])
            ...
            for j in range(self.nc):
                for k in range(j, self.nc):
                    g_Tp[j, k], = self.thermo.calc_bmcsl_gij_fmt(
                        n_alpha, self.mu_ij[j, k])
            self.T = T - T*eps
            self.calc_hs_diameters()
            self.calc_additive_diameters()
            ...
            for j in range(self.nc):
                for k in range(j, self.nc):
                    g_Tm[j, k], = self.thermo.calc_bmcsl_gij_fmt(
                        n_alpha, self.mu_ij[j, k])
            # Reset
            self.T = T
            self.calc_hs_diameters()
            ...
            for j in range(self.nc):
                for k in range(j, self.nc):
                    print("T: j,k", j, k,
                          (g_Tp[j, k]-g_Tm[j, k])/(2*T*eps), g_T[j, k])
...
if __name__ == "__main__":
    ...
    svrqm.set_tmin(2.0)
    svrqmf = saftvrqmie_functional(
        N=1, svrqm=svrqm, T_red=T/svrqm.eps_div_kb[0])
    #
    Tp = T + T*eps
    svrqmfp = saftvrqmie_functional(
        N=1, svrqm=svrqm, T_red=Tp/svrqm.eps_div_kb[0])
    #
    Tm = T - T*eps
    svrqmfm = saftvrqmie_functional(
        N=1, svrqm=svrqm, T_red=Tm/svrqm.eps_div_kb[0])
    for i in range(svrqm.nc):
        for j in range(svrqm.nc):
            print("i,j", i, j)
            print(
                "d_ij", (svrqmfp.d_ij[i, j]-svrqmfm.d_ij[i, j])/(2*T*eps), svrqmf.d_T_ij[i, j])
            print("delta_ij", (svrqmfp.delta_ij[i, j]-svrqmfm.delta_ij[i, j])/(
                2*T*eps), svrqmf.delta_T_ij[i, j])
            print(
                "mu_ij", (svrqmfp.mu_ij[i, j]-svrqmfm.mu_ij[i, j])/(2*T*eps), svrqmf.mu_ij_T[i, j])
    ...
    print("z thermopack", vle.pressure() *
          vle.liquid.specific_volume()/(svrqm.Rgas*T))
